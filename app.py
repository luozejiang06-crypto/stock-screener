...
def get_company_summary_zh(ticker):
    try:
        ...
        trimmed = raw_summary[:300] + "..." if len(raw_summary) > 300 else raw_summary
        
        translated = ""
        # 优先使用无需翻墙、国内直连的 MyMemory 翻译源
        try:
...
if selected_ticker:
    full_hist, stock_info = fetch_stock_deep_data(selected_ticker, selected_period)
    ...
    c_sub2.metric("RSI-14 (超买/超卖)", rsi_str, help="RSI>70 进入超买区，RSI<30 进入超卖区")
